...
const backgroundScrubFactory = (
	...
	const initScrubMeasurements = () => {
		...
		const viewPortHeight = window!.document.documentElement.clientHeight - wixAdsHeight
		const viewPortWidth = window!.document.documentElement.clientWidth
		Object.assign(measurements, { scrollY, siteHeight, wixAdsHeight, viewPortHeight, viewPortWidth })
	...
	const createScrubSequenceInstances = (animationManagerInstance: AnimatorManager) => {
		Object.entries(pageFeatureConfig.scrubSequencesParams).forEach(([sequenceId, sequenceParams]) => {
			const {
				...
				speedFactor,
				delay,
				breakpointsToExclude,
			} = sequenceParams
			const { viewPortHeight, viewPortWidth, components, siteHeight, wixAdsHeight } = measurements

			if (
				breakpointsToExclude &&
				breakpointsToExclude.some(
					(breakpoint) =>
						(!breakpoint.max || breakpoint.max >= viewPortWidth) && viewPortWidth >= breakpoint.min
				)
			) {
				const extra = Object.values(extraAnimationSelectors || {}) as Array<string>
				animationManagerInstance.runAnimation(getScrubClear([targetElementSelector, ...extra]))
				return
			}
			...
			sequenceFactories[sequenceId] = () =>
				animationManagerInstance.runAnimation({
					...
					params: {
						...
						viewPortHeight:
							viewPortHeight +
							(animationName === 'ImageReveal' || animationName === 'ImageParallax' ? wixAdsHeight : 0),
						componentHeight,
						componentTop,
						browserFlags,
	...
	const scrubProgress = () => {
		const { scrollY, viewPortHeight, components, wixAdsHeight } = measurements
		Object.entries(pageFeatureConfig.scrubSequencesParams).forEach(([sequenceId, sequenceParams]) => {
