...
export class NavigateServiceSdk extends WixStoresServiceSdk {
	...
	toProduct(productId: string) {
		this.fedopsLogger.interactionStarted(WixStoresNavigateSdkInteraction.NAVIGATE_TO_PRODUCT)
		return this.appsPublicApisUtils.getPublicAPI(STORES_APP_DEF_ID).then(async (api: any) => {
			await api.navigate.toProductPage({ id: productId })
			this.fedopsLogger.interactionEnded(WixStoresNavigateSdkInteraction.NAVIGATE_TO_PRODUCT)
		})
	}
}
