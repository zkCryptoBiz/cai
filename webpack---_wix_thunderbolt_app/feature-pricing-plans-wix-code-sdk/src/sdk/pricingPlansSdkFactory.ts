...
import type { Sorting } from '@wix/ambassador-pricing-plans-member-orders/types'
import { isArray, isPlainObject, mapKeys, transform } from 'lodash'
import type { PricingPlansPublicAPI, PricingPageOptions, NavigateToCheckoutOptions } from '@wix/pricing-plans-tpa-api'
...
export function PricingPlansSdkFactory({
	...
	const { sessionService, appsPublicApisUtils } = platformUtils
	...
	const pricingPlansSdk: PricingPlansWixCodeSdkWixCodeApi = {
		...
		customPurchaseFlow: {
			getPricingPageOptions() {
				return appsPublicApisUtils
					.getPublicAPI(APP_DEF_ID)
					.then((publicApi) => (publicApi as PricingPlansPublicAPI).getPricingPageOptions())
			},
			navigateToPricingPage(options: PricingPageOptions) {
				appsPublicApisUtils
					.getPublicAPI(APP_DEF_ID)
					.then((publicApi) => (publicApi as PricingPlansPublicAPI).navigateToPricingPage(options))
			},
			navigateToCheckout(options: NavigateToCheckoutOptions) {
				appsPublicApisUtils
					.getPublicAPI(APP_DEF_ID)
					.then((publicApi) => (publicApi as PricingPlansPublicAPI).navigateToCheckout(options))
			},
		},
	}
