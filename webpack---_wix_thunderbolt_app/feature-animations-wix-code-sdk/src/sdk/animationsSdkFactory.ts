	...
	return {
		[namespace]: {
			timeline: (timelineOptions: TimelineOptions = {}) => {
				...
				void createTimeline(timelineId, buildTimelineParams(timelineOptions, timelineId))
				const timeline: Timeline = {
					add: (comps, animations = [], offset) => {
						...
						void addToTimeline(timelineId, compIds, params, validOffset)
					...
					replay: () => {
						...
						if (!state.animatable) {
							...
							void seekTimeline(timelineId, 0)
							void playTimeline(timelineId)
						}
						return timeline
					},
					reverse: () => {
						...
						if (!state.animatable) {
							...
							void reverseTimeline(timelineId)
						}
						return timeline
					},
					onStart: (handler) => {
					...
					onComplete: (handler) => {
						if (handler) {
							...
							void onCompleteTimeline(timelineId)
						}
						return timeline
					},
					onRepeat: (handler) => {
						if (handler) {
							...
							void onRepeatTimeline(timelineId)
						}
						return timeline
					},
					onReverseComplete: (handler) => {
						if (handler) {
							...
							void onReverseCompleteTimeline(timelineId)
						}
						return timeline
					},
				}
				...
				void onStartTimeline(timelineId)
				return timeline
			},
		},
	}
