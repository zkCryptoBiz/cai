...
import { Duplexer as DuplexerType, InstanceUpdater } from '@wix/duplexer-js'
import {
	Channel,
	MessageHandler,
	Subscription,
...
export function realtime(
	...
	isAddRevisionToChannelNameInRealtimeEnabled: boolean
): RealtimeAPI {
	...
	async function createConnection() {
		...
		const instanceUpdater: InstanceUpdater = {
			...
			getHeaders: () => environment.getCommonConfigHeader(),
		}
	...
	function maybePrefix(channel: Channel): Channel {
		...
		} else if (isAddRevisionToChannelNameInRealtimeEnabled) {
			return {
				name: `${environment.getSiteRevision()}-${channel.name}`,
				resourceId: channel.resourceId,
			}
		}
		return channel
