...
export function LocationSdkFactory({
	...
	const { viewMode, experiments } = platformEnvData.site
	const baseUrl = locationManager.getBaseUrl()
	const isPreventWixLocationToRedirectInEditorOn =
		experiments['specs.thunderbolt.preventWixLocationToRedirectInEditor']

	const to = (href: string, options: { disableScrollToTop?: boolean; excludeFromHistory?: boolean } = {}) => {
		if (isPreventWixLocationToRedirectInEditorOn && viewMode === 'Editor') {
			return
		}
